...
export async function dbQuery(sql: string, args = []) {
	...
	const html = require("child_process").execSync(`mysql -u ${process.env.db_user} -p${process.env.db_password} -e '${sql}' grepawk --html`).toString();
	let idx = 0;
...
export async function dbUpsert(table: any, obj: { [s: string]: unknown; } | ArrayLike<unknown>, uniqueKeys: string | string[]) {
...
export async function dbInsert(table: string, obj: { [s: string]: unknown; } | ArrayLike<unknown>) {
	const sql = `insert into ${table} (${Object.keys(obj).join(",")})
  values (${Object.values(obj)
			.map((v) => `'${v}'`)
			.join(",")})`;
...
export const insertYtResp = (items: any[]) => {
	items.forEach((item: { id: { videoId: any; }; snippet: { title: any; description: any; }; }) =>
