...
function listFiles(containerName, prefix = null) {
	return new Promise((resolve, reject) => {
		...
		var _page_through = function (containerName, nextPage) {
			blobClient.listBlobsSegmentedWithPrefix(
				containerName,
				prefix,
				nextPage,
				(err, result) => {
...
function listContainers(prefix = "") {
	return new Promise((resolve, reject) => {
		blobClient.listContainersSegmentedWithPrefix(
			prefix,
			null,
			(err, result) => {
				if (err) reject(err);
...
async function fopen(xpath: string): Promise<AzureFileDriver | any> {
	...
	try {
		...
		function download(downlink: Writable) {
			const fh = blobClient
				.createReadStream(container, blobName, (err, fileInfo) => {
					if (err) throw err;
				})
				.pipe(downlink);
		}
...
function fopenr(xpath: string): Promise<Stream.Readable | any> {
	...
	let fh = new Promise((resolve, reject) => {
		blobClient.createReadStream(
			container,
			blobname,
			options,
			(err, result, resp) => {
...
function fopenw(xpath: string): Promise<Stream.Writable> {
	return new Promise((resolve, reject) => {
		...
		const container = parts[0] ? parts[0] : "public_file";
		const blobname = parts[1] ? parts[1] : "new_file.txt";
		const writeStream = blobClient.createWriteStreamToBlockBlob(
			container,
			blobname,
			(err, result, resp) => {
				if (err) reject(err);
...
function file_put_content(containerName, blobName, text) {
	return new Promise((resolve, reject) => {
		blobClient.createBlockBlobFromText(containerName, blobName, text, function (
			err,
			result,
			response
		) {
